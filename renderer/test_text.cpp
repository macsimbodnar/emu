...
void loop_drawSomeText() {
    ...
    while (!done) {
        while (SDL_PollEvent(&event)) {
            ...
            { done = 1; }
                ...
                if (FC_InRect(event.button.x, event.button.y, input_rect)) {
                    input_position = FC_GetPositionFromOffset(font, event.button.x - input_rect.x,
                                     event.button.y - input_rect.y, input_rect.w, FC_ALIGN_LEFT, "%s", input_text);
                }
            }
        }
        ...
        if (mode == 0 || mode == 1) {
            ...
            FC_DrawColor(font, target, rightHalf.x, 65, FC_MakeColor(128 + 127 * sin(time),
                         128 + 127 * sin(time / 2), 128 + 127 * sin(time / 4), 128 + 127 * sin(time / 8)),
                         "Dynamic colored text");
            FC_Draw(font, target, rightHalf.x, 85, "Multi\nline\ntext");

            FC_DrawBox(font, target, input_rect, "%s", input_text);
            draw_rect(FC_MakeRect(input_rect.x, input_rect.y + FC_GetLineHeight(font), FC_GetWidth(font, "%s",
                                  input_text), 2), black);

            FC_Rect input_cursor_pos = FC_GetCharacterOffset(font, input_position, input_rect.w, "%s",
                                       input_text);
