...
static bool load_NES_cartridge(const char *file,
                               NES_cartridge_t &cartridge_out);
...
TEST_CASE("NES Test") {
  ...
  // Load the NES test cartridge. Execute it at address 0xC000 and compare the
  // log with the log_file
  ...
  // Reset the cpu before use and set the Program Counter to specific mem addres
  // in order to perfrom all tests
  cpu.reset();
  ...
  while (log_file) {
    ...
    if (log_file) {
      ...
      // Compare registers
      cmp_res = memcmp(line + LOG_REG_OFFSET, state_log + LOG_REG_OFFSET,
                       LOG_REG_LEN);
      ...
      cmp_res = memcmp(line + LOG_CYC_OFFSET, state_log + LOG_CYC_OFFSET,
                       len - LOG_CYC_OFFSET);
...
TEST_CASE("Cycles Timing Test") {
  ...
  // Initialize the cpu and set the log callback
  MOS6502 cpu(
      [](void *usr_data, const uint16_t address, const access_mode_t read_write,
         uint8_t &data) -> void {
  ...
  mem[0xFFFC] =
      (uint8_t)TIMING_TEST_MEM_LOC & 0x00FF; // Set the reset Vector ll
  mem[0xFFFD] =
      (uint8_t)(TIMING_TEST_MEM_LOC >> 8) & 0x00FF; // Set the reset Vector hh

  // NOTE(max):   Using the srec from this discussion
  // http://forum.6502.org/viewtopic.php?f=8&t=3340
...
static void log_clb(const std::string &log) { printf("%s\n", log.c_str()); }
...
static void mem_callback(void *usr_data, const uint16_t address,
  ...
  if (address >= 0x8000 && address <= 0xFFFF) {
    uint16_t mapped_addr =
        address & (cartridge->prg_banks > 1 ? 0x7FFF : 0x3FFF);
  ...
  // If here means the address is have to be handled by the PPU. Is ignored for
  // this test
  if (address >= 0x2000 && address <= 0x3FFF) {
    log_clb("PPU mem access. Access type: " +
            ((read_write == access_mode_t::READ) ? std::string("READ")
                                                 : std::string("WRITE")) +
...
static bool load_NES_cartridge(const char *file,
  ...
  // Read the header
  size_t b_readen =
      fread((char *)&header, sizeof(char), sizeof(ines_header_t), fp);
  ...
  // Determine Mapper ID
  cartridge_out.mapper_id =
      ((header.mapper2 >> 4) << 4) | (header.mapper1 >> 4);
  cartridge_out.mirror =
      (header.mapper1 & 0x01) ? mirror_t::VERTICAL : mirror_t::HORIZONTAL;
  ...
  size_t read_res = fread((uint8_t *)cartridge_out.prg_memory.data(),
                          sizeof(uint8_t), cartridge_out.prg_memory.size(), fp);

  if (read_res != cartridge_out.prg_memory.size()) {
    log_clb("Failed read the cartridge program memory from file: " +
            std::string(file));
    fclose(fp);
    return false;
  }
  ...
  read_res = fread((uint8_t *)cartridge_out.chr_memory.data(), sizeof(uint8_t),
                   cartridge_out.chr_memory.size(), fp);

  if (read_res != cartridge_out.chr_memory.size()) {
    log_clb("Failed read the cartridge character memory from file: " +
            std::string(file));
    fclose(fp);
    return false;
  }
